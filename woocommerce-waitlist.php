...
if ( ! class_exists( 'WooCommerce_Waitlist_Plugin' ) ) {
	...
	class WooCommerce_Waitlist_Plugin {
		...
		public function perform_api_mailout_bundles( $product, $updated_props ) {
			if ( $product->is_type( 'bundle' ) && in_array( 'bundled_items_stock_status', $updated_props, true ) && $product->is_in_stock() && 'publish' === get_post_status( $product->get_id() ) ) {
				$this->do_mailout( $product );
			}
		}
		...
		public function stock_level_has_broken_threshold( $product, $stock_level_required ) {
			$previous_stock_level = get_post_meta( $product->get_id(), 'wcwl_stock_level', true ) ? absint( get_post_meta( $product->get_id(), 'wcwl_stock_level', true ) ) : 0;
